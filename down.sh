...
HOOKS=$(curl -s -H "Accept: application/json; api-version=1.0" -H "Content-Type:application/json" -XGET -u :$TFS_TOKEN $HOOK_URL | jq -c '.value[] | select(.publisherInputs.branch | contains("$BRANCH"))')
echo "HOOKS: $HOOKS"
